...
OUTPUT_files=ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OUTPUTS_4p1000_v8/AMG/'
...
def J_new(in_new):
	...
	prop_straw = np.minimum(litter_inc/in_new,1.)
	print '######################'
	print 'prop_straw',prop_straw
	humcoef_pred=np.zeros(2)
	humcoef_pred[1]=humcoef[1]
	humcoef_pred[0] = humcoef[0]*prop_straw[0]+humcoef_OM*(1.-prop_straw[0])
	print 'humcoef_pred',humcoef_pred
	print '######################'
...
if(optimization_4p1000):
	...
	##################
	n=0 # contatore
	inp=open(ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/optim_param_AMG.txt','rb')
	while inp.read(1):
		inp.seek(-1,1)
...
for site in site_names_all:
	...
	date_init_ss = np.str(np.int(year_0 - 30))
        ...
        clay = np.mean(site_T0['Clay'])*1000.
