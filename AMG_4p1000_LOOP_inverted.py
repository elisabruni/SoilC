...
if(optimization_param):
	out1=open("SOC_AMG_optim2.txt","wb")
if(optimization_4p1000):
        out2=open("SOC_AMG_4p1000.txt","wb")
...
#############################################
#def forward_ACT(n_an,init,litterin,CN_crop,temp_mean,water_mean,pot_evapot,k0,history_land):
...
#############################################
	...
	#humcoef=humcoef_func(CN_crop)
	#Average humcoef for crops in the database (AG and BG)
	#humcoef = np.array([0.253,0.4])
	#TestMin humcoef rotazioni
	#humcoef = np.array([0.217,0.4])
	#TestMax humcoef rotazioni
        #humcoef = np.array([0.315,0.4])
...
def J_new(in_new):
	...
	#Active
	predict_c_fwd_4p1000=forward_ACT(n_an_4p1000,SOC_data_init,in_new,temp_t,water_t,PET_t,optimized_k,arable_scoef,humcoef)
	#Stable
...
loc_exp = ROOTDIR+'SIMULAZIONI_ORCHIDEE_SITI/SITES_dataset_Century_new_AMG.xlsx'
...
if(optimization_4p1000):
	...
	##################
	n=0 # contatore
	inp=open('opt2_k_forscriptAMG.txt','rb')
	while inp.read(1):
		inp.seek(-1,1)
...
for j in range(N_sites):
	...
	print SOC_data_init
        ...
        #if(not optimization_4p1000):
        #        Dynamics_all=np.concatenate((np.array([SOC_data_init]),SOC_tot))
        #        print Dynamics_all
        #        print "--"
