...
loc_out = ROOTDIR+"SCRIPT_MODELLI/MULTIMODEL/OUTPUTS_4p1000_v8/MIMICS/"
loc_exp = ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/SITES_dataset_multimodel.xlsx'
loc_optim_param = ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/'
loc_spinup = ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/'
...
#Control plots
site_T0_array_all=np.array(['CHNO3_Min', 'COL_T0', 'CREC3_Min', 'FEU_T0', 'LAJA2_Min',
...
for site in site_names_all:
        ...
        with open(soil_temp) as fileID:
                # C
                temp_tin = np.array(map(float,fileID))
                #SITE_temp_t.append(temp_tin)

	TSOI_mean_fw = DailyToYearly(temp_tin,1)
	...
	append_temp.append(rep) #ripeti ultimo anno per siti con 30 anni
...
for j in range(N_sites):
	...
	t_fwd=np.arange(0,n_days_4p1000)
        #t_fwd=np.arange(0,n_days_4p1000-365)
        fwd_4p1000 = odeint(XEQ_fw,state,t_fwd,args=(I,))

        target = np.sum(fwd_4p1000[0,:])*0.004
        print 'target:',target
	...
	SOC_model_standard_pools = predict_c_standard_pools
	SOC_model_standard = predict_c_standard
	...
	SOC_model_opt_pools = predict_c_opt_pools
	SOC_model_opt = predict_c_opt
