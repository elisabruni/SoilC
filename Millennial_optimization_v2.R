...
#OUTPUT FILES
location_dataframe<-"/Users/ebruni/Desktop/DOTTORATO/SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/SOC_MILLENNIALv2_optim.csv"
location_dataframe2<-"/Users/ebruni/Desktop/DOTTORATO/SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/optim_param_MILLENNIALv2.csv"
location_spinup_OPT<-"/Users/ebruni/Desktop/DOTTORATO/SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/list_SOC_spinup_opti_MILv2.csv"
location_spinup_nonOPT<-"/Users/ebruni/Desktop/DOTTORATO/SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/RUN_FWD8/list_SOC_spinup_nonOPTI_MILv2.csv"
location_plot<-"/Users/ebruni/Desktop/DOTTORATO/SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/GRAFICI_FIT/FIT_MODELLI/fit_MILLENNIAL_v2.pdf"
...
Jparam_new<-function(param_new){
  ...
  model_out_pred <- Model.fwd(param_list,run.steps,state.model)
...
#Model.fwd <- function (parameters, times=run.steps, y= state.SS) {
...
Model.fwd <- function (parameters, times, y) {
  output <- ode(y = y, times=times, func=decomp, parms =parameters, temp_func=forc_st_fw,water_func=forc_sw_fw, method="rk4") #solve ode, return output
  #print(length(forc_st_fw[[1]])/365)
  return(as.data.frame(cbind(time = output[run.steps.minus.one,"time"], POM = output[run.steps.minus.one,"POM"], LMWC = output[run.steps.minus.one,"LMWC"], AGG = output[run.steps.minus.one,"AGG"], MIC = output[run.steps.minus.one,"MIC"], MAOM = output[run.steps.minus.one,"MAOM"])))
}
...
for(site in site_names_all){
  ...
  #forward simu
  model_out <- Model.fwd(param_list,run.steps,state.SS)
...
for (site in site_names_all){
  ...
  #Araz works
  #optim_func<-optim(prior_params,Jparam_new, method = "L-BFGS-B",
  #                  lower = c(prior_param1-1e4,prior_param2-1e4,prior_param3-9e3), upper = c(prior_param1+1e5,prior_param2+1e5,prior_param3+1e5))
  #Araz and Avri work
  #optim_func<-optim(prior_params,Jparam_new, method = "L-BFGS-B",
  #                                    lower = c(prior_param1-1e4,prior_param2-1e4,prior_param3-9e3), upper = c(prior_param1+8000,prior_param2+8000,prior_param3+1e3))
  optim_func<-optim(prior_params,Jparam_new, method = "L-BFGS-B",
                    lower = c(prior_param1-1e4,prior_param2-1e4,prior_param3-9e3), upper = c(prior_param1+7000,prior_param2+7000,prior_param3+1e3))
  ...
  #Forward
  model_out_opt <- Model.fwd(param_list_opt,run.steps,state.model_opt)
