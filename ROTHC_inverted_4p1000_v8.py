...
OUTPUT_files=ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OUTPUTS_4p1000_v8/ROTHC/'
...
#A_matrix
############################################################
def A_matrix(clay,litterin,DPM_RPM_HUM_frac_inputs):
	
	alpha = BIO_frac_fromRPM*BIO_HUM_partition(clay)
	beta = HUM_frac_fromRPM*BIO_HUM_partition(clay)+litterin[2]*DPM_RPM_HUM_frac_inputs[2]
...
#Spinup
############################################################
def spinup(SOC_data_init,litterin,clay,temp,rain, pot_evapot,c_param,t_param,param_temp_in,DPM_RPM_HUM_frac_inputs):
    ...
    a_ma_mean=A_matrix(clay,litterin,DPM_RPM_HUM_frac_inputs)
...
#Forward
############################################################
def forward_ACTIVE(SOC_data_init,n_an,init,litterin,clay,temp,water,mean_pot_evapot,c_param,t_param,param_temp_in,DPM_RPM_HUM_frac_inputs):
    ...
    a_ma=A_matrix(clay,litterin,DPM_RPM_HUM_frac_inputs)
...
def Jparam_new(param_new_t):
    ...
    global clay,temp_t,water_t,PET_t,ssp_select,DPM_RPM_HUM_frac
    ...
    param_predict_c = spinup(SOC_data_init,litter_inc,clay,temp_in,water_in, PET_in,c_vegetated,t_param,param_new_t,DPM_RPM_HUM_frac)
    ssp = np.sum(param_predict_c)

    param_predict_c_forward = forward_ACTIVE(SOC_data_init,n_an,param_predict_c,litter_inc,clay,temp_t,water_t,PET_t,c_vegetated,t_param,param_new_t,DPM_RPM_HUM_frac)
    ssp_fwd = np.sum(param_predict_c_forward,axis=1)


    #selct model values where data avail
    ssp_select=ssp_fwd[ss0>0]

    #print 'ss0',ss0
    #print 'SOC_data',SOC_data
    #print 'model all', ssp_fwd
...
def J_new(in_new):
        ...
        global NEW_ITER
	global n_an_4p1000, c_vegetated, DPM_RPM_HUM_frac
        global spinup_c_4p1000, predict_c
	global clay,temp_t,water_t,PET_t,c_vegetated,t_param
	global DPM_RPM_HUM_frac_mix, litter_mean

        #Calculate proportion of straw / OM
        prop_straw = np.minimum(litter_mean/in_new,1.)
	DPM_RPM_HUM_frac_mix = DPM_RPM_HUM_frac*prop_straw+DPM_RPM_HUM_frac_FYM*(1.-prop_straw)

	litter_new = np.zeros(3)
        litter_new[0] = DPM_RPM_HUM_frac_mix[0]*in_new
        litter_new[1] = DPM_RPM_HUM_frac_mix[1]*in_new
        litter_new[2] = DPM_RPM_HUM_frac_mix[2]*in_new

        #print ' '
        #print '######################'
        #print 'prop_straw',prop_straw
	#print 'previous DPM_RPM_HUM_frac (plant):'
	#print ' '
	#print DPM_RPM_HUM_frac
	...
	predict_c_pools  = forward_ACTIVE(SOC_data_init,n_an_4p1000,spinup_c_4p1000,litter_new,clay,temp_t,water_t,PET_t,c_vegetated,t_param,optimized_t_param,DPM_RPM_HUM_frac_mix)
...
SITE_Err2_litter_arr = np.zeros((N_sites,3)) #array
SITE_water_in=[]
SITE_temp_in=[]
SITE_PET_in=[]
SITE_water_t=[]
...
for site in site_names_all:
	...
	HUM_m = LITTER_mean*HUM_frac_plant #=0 for plants
	
	DPM_var = LITTER_var*DPM_frac_plant*DPM_frac_plant
	RPM_var = LITTER_var*RPM_frac_plant*RPM_frac_plant
	HUM_var = LITTER_var*HUM_frac_plant*HUM_frac_plant

        litter_inc = np.array([DPM_m,RPM_m,HUM_m])    # litter C inputs parameters (tC/ha/month)
	Err2_litter_inc_arr = np.array([DPM_var,RPM_var,HUM_var])
...
for j in range(N_sites):
	...
	PET_t     = SITE_PET_t[j]
        ...
        frac_array=DPM_RPM_HUM_frac
        print 'DPM:RPM and HUM fractions',frac_array
        if(optimization_4p1000):
	...
	#litter_inc in DPM and RPM
	litter_inc=np.array([litter_mean*frac_array[0],litter_mean*frac_array[1],litter_mean*frac_array[2]])
	...
	print '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>'
        ...
        #--------------------------------------------------------------#
	if(optimization_param):
        	spinup_c=spinup(SOC_data_init,litter_inc,clay,temp_in,water_in, PET_in,c_vegetated,t_param,prior_param_temp,DPM_RPM_HUM_frac)
		#Standard forward (non optimzed)
        	fwd=forward_ACTIVE(SOC_data_init,n_an,spinup_c,litter_inc,clay,temp_t,water_t,PET_t,c_vegetated,t_param,prior_param_temp,DPM_RPM_HUM_frac)

		#First year of experim
		SITE_SOC_model=np.sum(fwd[0,:]) #tC/ha

		print 'Stationary solution before optimiz: '
		print np.sum(spinup_c)
		...
		print fwd
		print "--"
		Dynamics_select_exper = fwd[ss0>0]
		Dynamics_select_exper_sum = np.sum(Dynamics_select_exper,axis=1)
		print Dynamics_select_exper_sum

	if(optimization_4p1000):
                spinup_c_4p1000=spinup(SOC_data_init,litter_inc,clay,temp_in,water_in, PET_in,c_vegetated,t_param,optimized_t_param,DPM_RPM_HUM_frac)
                fwd_4p1000=forward_ACTIVE(SOC_data_init,n_an_4p1000,spinup_c_4p1000,litter_inc,clay,temp_t,water_t,PET_t,c_vegetated,t_param,optimized_t_param,DPM_RPM_HUM_frac)
		SITE_SOC_model=np.sum(fwd_4p1000[0,:]) #tC/ha
	...
	if(optimization_param):
		...
		bnds_param=[(10.,30.)]
		opt_param_mean=minimize(Jparam_new, prior_param_temp, method='SLSQP', bounds=bnds_param, options={'disp':True,'maxiter':5000})
	...
	if(optimization_4p1000):
		...
		############################################################
                ...
                ###########################################################
                predict_c_standard_pools=forward_ACTIVE(SOC_data_init,n_an_4p1000,spinup_c_4p1000,litter_inc,clay,temp_t,water_t,PET_t,c_vegetated,t_param,optimized_t_param,DPM_RPM_HUM_frac)
		predict_c_standard = np.sum(predict_c_standard_pools,axis=1)

                #4p1000
		predict_c_opt_pools=forward_ACTIVE(SOC_data_init,n_an_4p1000,spinup_c_4p1000,litter_opt_array,clay,temp_t,water_t,PET_t,c_vegetated,t_param,optimized_t_param,DPM_RPM_HUM_frac_mix)
		...
		############################
                ...
                if(Uncert_Q):
					...
					in_rand_tot = np.sum(in_rand) #1D->splitted in Jnew
                                        in_rand_param=np.array([in_rand_tot*DPM_RPM_HUM_frac_mix[0],in_rand_tot*DPM_RPM_HUM_frac_mix[1],in_rand_tot*DPM_RPM_HUM_frac_mix[2]]) #3D
					#print '((((((((((((((((()))))))))))))))))'
					...
					#print in_rand_param*12.
                                        ...
                                        opt_tot=opt.x[0] #1D
					#print opt_tot
					opt_param = np.array([opt_tot*DPM_RPM_HUM_frac_mix[0],opt_tot*DPM_RPM_HUM_frac_mix[1],opt_tot*DPM_RPM_HUM_frac_mix[2]]) #3D
					#print '((((((((((((((((()))))))))))))))))'
			...
			litter_opt_errHUM=np.std(opt_parameters_MC[:,2])/np.sqrt(MC_length)
                        litter_opt_err = np.array([litter_opt_errDPM,litter_opt_errRPM,litter_opt_errHUM])
