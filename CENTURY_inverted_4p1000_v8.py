...
OUTPUT_files=ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OUTPUTS_4p1000_v8/CENTURY/'
...
site_T0_array_all=npy.array(['CHNO3_Min', 'COL_T0', 'CREC3_Min', 'FEU_T0', 'LAJA2_Min', 'RHEU1_Min', 'RHEU2_T0','ARAZ_D0_N0', 'ULT_P0_B', 'BROAD_3_Nill', 'TREV1_Min','AVRI_T1TR','BOLO_T0', 'GRAB_CP','MUNCHE_CP','RITZ_CP'])
...
#################
n=0 # contatore
inp=open(ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/optim1_param_CENTURY.txt','rb')
while inp.read(1):
    inp.seek(-1,1)
...
#################
n=0 # contatore
inp=open(ROOTDIR+'SCRIPT_MODELLI/MULTIMODEL/OPTIM_MODELS/OPTIM8/optim2_param_CENTURY.txt','rb')
...
out_tr=open(OUTPUT_files+"SOC_experiments.txt","wb")
...
for site in site_names:
	...
	npy.save(out_tr,TREATMENTS)
        ...
        #SOC_dyn= site_T0['SOC']*100 #(gC/m2)
	#LINEAR INTEPOLATION
        #SOC_dyn_int = SOC_dyn.astype(float).interpolate()
        #SOC_dyn_int = pd.DataFrame(SOC_dyn_int).fillna(method = 'bfill')
        #SOC_dyn_int = SOC_dyn_int.values.flatten()
        #SOC_exp_array.append(npy.asarray(SOC_dyn_int))
...
def forward(n_an,init,litterin,tau_array,clay,temp,water, soil_Q10):
    ...
    matrix_cpools_tmean = npy.zeros((n_an,np))
...
out_mo_pools=open(OUTPUT_files+"SOC_CENTURY_pools.txt","wb")
out_mo=open(OUTPUT_files+"SOC_CENTURY.txt","wb")
out_lit=open(OUTPUT_files+"Litter_income_CENTURY.txt","wb")
out_lit_tot=open(OUTPUT_files+"Litter_tot_CENTURY.txt","wb")
out_priors = open(OUTPUT_files+"priors_and_opt_in_CENTURY.txt","wb")
...
for j in range(N_sites):
	...
	#to be saved	
	litter_inc_save=npy.append(litter_inc*365./100.,tot_litter_inc*365./100.) #tC/ha/yr
	litter_inc_err_save=npy.append(npy.sqrt(Err2_litter_inc)*365./100.,npy.sqrt(tot_err2_litter_inc)*365./100.) #CHECK!!
	...
	in_opt = npy.array(litter_inc)*(1+0.004)
        ...
        n_an = 30
	fwd=forward(n_an,spinup_c,frac_array,prior_tau,clay,temp_t,water_t,Q10_Tref)
        #--------------------------------------------------------------#
        SITE_SOC_model=npy.sum(fwd[0,:]) #gC/m2
        print 'Stationary solution before 4x1000: '
        print npy.sum(spinup_c)
	...
	litter_opt = opt_mean.x
        print "SLSQP: Optimum solution (gC/m2/day):", litter_opt
        total_opt_in=npy.sum(opt_mean.x)

	#optimized litter pools and total litter (save)
	in_opt_save = npy.append(litter_opt*365./100.,total_opt_in*365./100) #tC/ha/yr
	...
	SOC_pools_out=npy.stack((predict_c_standard_pools/100.,predict_c_opt_pools/100.))
	SOC_out=npy.stack((year_out,predict_c_standard/100.,predict_c_opt/100.)) #tC/ha
	...
	Uncert_Q = True
		...
		out_priors_and_opt = npy.stack((in_rand_param_MC*365./100.,opt_parameters_MC*365./100.)) #save as tC/ha/yr
		...
		in_opt_err_save=npy.append(unc_litter_opt*365./100.,unc_litter_opt_sum*365./100.) #tC/ha/yr CHECK !!
		...
		litter_tot_save = npy.stack((tot_litter_inc*365./100.,npy.sqrt(tot_err2_litter_inc)*365./100.,total_opt_in*365./100,unc_litter_opt_sum*365./100.)) #tC/ha/yr
		npy.save(out_lit_tot,litter_tot_save)
...
opt_out=open(OUTPUT_files+"opt_litterin_CENTURY.txt","wb")
