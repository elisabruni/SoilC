...
out=open("SOC_RothC_optim6.txt","wb")

plot_CM=False
if(plot_CM):
	pp0 = PdfPages("control_moisture_sites.pdf")
...
def K_matrix(clay,temp_mean,rain, mean_pot_evapot,c_param,t_param):
	global moist_rate_b, accTSMD_abs
	K_matrix=np.zeros((4,4))
	moist_rate_b, accTSMD_abs = control_moist_func(clay, rain, mean_pot_evapot,c_param)
	K_matrix[0,0]=kDPM*control_temp_func(temp_mean)*moist_rate_b*c_param*t_param
	K_matrix[1,1]=kRPM*control_temp_func(temp_mean)*moist_rate_b*c_param*t_param
	K_matrix[2,2]=kBIO*control_temp_func(temp_mean)*moist_rate_b*c_param*t_param
	K_matrix[3,3]=kHUM*control_temp_func(temp_mean)*moist_rate_b*c_param*t_param
	K_out = K_matrix
	return K_out
