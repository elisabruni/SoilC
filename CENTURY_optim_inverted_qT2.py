...
outSOC=open("SOC_CENTURY_optim.txt","wb")

##################
#open optimized struc:metab ratios
#################
n=0 # contatore
#inp=open('opt_abfractions_Century_multimodel.txt','rb')
inp=open('optim1_param_CENTURY.txt','rb')
while inp.read(1):
    inp.seek(-1,1)
    XXmet=npy.load(inp)
n+=1
...
def Jq10_new(q10_new_t):
    ...
    Delta = q10_predict_c_forward-SOC_data
...
#########
min_soilQ10=0. #ln(1)
max_soilQ10=npy.log(5)

bnds=[(min_soilQ10,max_soilQ10),(0.,30.)]
...
for j in range(N_sites):
	...
	XXmet_nonOPT = npy.array([0.6916,0.6916])
	XXstr_nonOPT=1-XXmet_nonOPT

	imp_frac_nonOPT = npy.zeros(4)
	imp_frac_nonOPT[0]=XXstr_nonOPT[0]
	imp_frac_nonOPT[1]=XXmet_nonOPT[0]
	imp_frac_nonOPT[2]=XXstr_nonOPT[1]
	imp_frac_nonOPT[3]=XXmet_nonOPT[1]

        ss=spinup(prior_tau,imp_frac_nonOPT,clay,temp_in,water_in,prior_soilQ10_t)
        fwd=forward(YEARS,ss,imp_frac_nonOPT,prior_tau,clay,temp_t,water_t,prior_soilQ10_t)
        #--------------------------------------------------------------#
	...
	ssp_o = npy.array([ssp])
        SOC_model = q10_predict_c_forward
	SOC_error=SOC_var
	print 'SOC_error',SOC_error
	#XX=npy.stack((YEARS,SOC_model,SOC_data,SOC_error))

	XX=npy.stack((YEARS,SITE_SOC_dyn/100.,SOC_model/100.,SOC_data/100.,SOC_error/10000.)) #tC/ha/yr
	npy.save(outSOC,XX)
...
#out2=open("opt_q10Tref_multimodel.txt","wb")
out2=open("optim2_param_CENTURY.txt","wb")
npy.save(out2,q10_sites)
out2.close()

#optimized_values=q10_sites[~npy.all(q10_sites==0,axis=1)]
#opt_out=open("opt_q10_tref2.11.txt","wb")
